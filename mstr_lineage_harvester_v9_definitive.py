...
TARGET_PROJECT   = "YOUR_DEV_PROJECT_NAME"
TARGET_FOLDER_ID = ""
...
def main():
    t0 = datetime.now()
    print("=" * 65)
    print("  MSTR LINEAGE HARVESTER  v9.0.0  (mstrio-py definitive)")
    ...
    conn = Connection(SOURCE_BASE_URL, MSTR_USERNAME, MSTR_PASSWORD,
                      project_name=SOURCE_PROJECT, login_mode=1,
                      ssl_verify=False)
    pid = conn.project_id
    log.info(f"  Project ID: {pid}")

    h = LineageHarvester(conn, pid, SOURCE_PROJECT)
    h.harvest_l1()        # Documents & Dossiers
    h.harvest_l2()        # Datasets + L12 + Cube SQL
    h.harvest_l3()        # Report objects + L23
    h.harvest_l4()        # Schema objects from logical tables
    h.harvest_l34()       # Metric -> schema mapping
    df = h.build_flat_table()  # Join everything -> 24 columns
    ...
    # CSV export
    if CSV_EXPORT:
        cp = f"lineage_{SOURCE_PROJECT}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    ...
    tgt = Connection(TARGET_BASE_URL, MSTR_USERNAME, MSTR_PASSWORD,
                     project_name=TARGET_PROJECT, login_mode=1,
                     ssl_verify=False)

    ds = SuperCube(connection=tgt, name=CUBE_NAME)
